...
export default function AddStockModal({
  ...
  const [stockType, setStockType] = useState<StockTypeValue>('' as StockTypeValue);
  const [region, setRegion] = useState<RegionValue>('' as RegionValue);
  const [stockTrend, setStockTrend] = useState<StockTrendValue>(null);
  const [marketCategory, setMarketCategory] = useState<MarketCategoryValue>('' as MarketCategoryValue);
  ...
  React.useEffect(() => {
    if (isOpen) {
      setSymbol('');
      setStockType('' as StockTypeValue);
      setRegion('' as RegionValue);
      setStockTrend(null);
      setMarketCategory('' as MarketCategoryValue);
      ...
      setSwingHoldRatio('');
      setStockCommission('');
      setHtp(''); // Reset to empty since it's now optional
      setError(null);
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const stockDataPayload = {
      ...
      htp: htpValue, // Optional field, can be null
    };
